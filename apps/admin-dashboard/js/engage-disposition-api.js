...
(function () {
  ...
  function agentLabel(agent) {
    ...
    return String(agent?.userId || 'Membro');
  }
  ...
  async function assignAgent(session, tenantId, conversationId, agentUserId) {
    const tid = resolveTenantId(session, tenantId);
    if (!tid || !conversationId) throw new Error('Tenant ou conversa inválidos.');
    return apiPost(buildAssignPaths(tid, conversationId), {
      agentUserId: agentUserId == null ? null : String(agentUserId),
    });
  }
